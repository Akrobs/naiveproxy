...
✅ Новый GitHub-адрес `ivan-yurich/naiveproxy`
✅ `/reload` — применить Caddyfile без перезапуска Caddy
✅ Валидация логинов, паролей и доменов
✅ Безопасная генерация `basic_auth` в Caddyfile
✅ Fixed-string поиск пользователей в CLI и Telegram
✅ Безопасный DNS whitelist и парсинг blocklists
✅ Обновление Caddy собирает бинарник до короткого рестарта
✅ Синхронизация `files/naiveproxy.sh` с основной версией
✅ Синтаксическая проверка обеих копий скрипта
...
| `/reload` `/restart` `/update` `/selfupdate` | Управление |
...
| ⭐ **GitHub Star** | [Поставить звезду](https://github.com/ivan-yurich/naiveproxy) |
| 📱 **Telegram канал** | [t.me/ivan_it_net](https://t.me/ivan_it_net) |
| 🌐 **Сайт** | [ivan-it.net](https://ivan-it.net) |
...
<details>
<summary><b>v4.2.4</b> — Security Audit & Reload ← ТЕКУЩАЯ</summary>

- 🔗 Новый GitHub-адрес: `ivan-yurich/naiveproxy`
- ♻️ `reload` команда и `/reload` в Telegram без перезапуска Caddy
- 🔒 Валидация логинов, паролей, доменов и DNS whitelist
- 🔧 Безопасная генерация Caddyfile `basic_auth`
- ⚡ Обновление Caddy без 5-15 минут простоя на сборке
- 📦 `files/naiveproxy.sh` синхронизирован с основной версией
