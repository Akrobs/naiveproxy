...
**Полный аудит безопасности + рабочий Telegram бот + блокировщик рекламы**

</div>

| Блок | Что добавлено |
|------|---------------|
| 🤖 **Telegram бот** | 16 команд, мультиадмины, QR код картинкой, авто-установка зависимостей |
| 🚫 **DNS блокировка** | unbound + 1.5М доменов рекламы, DNS-over-TLS, whitelist |
| 🔍 **Диагностика** | 7 блоков, 18+ проверок, отчёт в Telegram |
| 🔒 **SSH Hardening** | ED25519, `sshd_config.d/` поддержка, `ssh.socket` отключение |
| 🛡️ **Защита** | Fail2Ban 3 уровня (`iptables-multiport`), UFW, защита последнего домена |
| ♻️ **Авто-восстановление** | `Restart=on-failure` для Caddy и бота |
| 🎨 **Брендинг** | ASCII баннер, Telegram канал, ivan-it.net |

### 🐛 Исправленные баги в этой версии:

- ✅ `/qr` команда — фикс curl `-F` + `--data-urlencode` конфликта
- ✅ `/adduser` — корректная валидация логина и пароля
- ✅ Команды бота чистятся от `\r\n` (Telegram добавлял невидимые символы)
- ✅ `set +e` в обработчике — ошибка не ломает бот
- ✅ Диагностика — счётчики `pass=$((pass+1))` вместо `((pass++))`
- ✅ ALPN проверка — добавлен `-servername` для SNI и флаг `-a` для бинарного вывода
- ✅ SSH порт меняется на Ubuntu 22.04+ (через `sshd_config.d/`)
- ✅ Защита от удаления последнего домена
- ✅ `apt update` перед установкой Fail2Ban
- ✅ Caddy и бот теперь авто-перезапускаются при сбое
